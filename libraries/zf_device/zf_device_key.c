...
// 函数简介     获取短按键值（无短按返回0）
// 参数说明     void
// 返回参数     uint8           0：无短按；1~KEY_NUMBER：对应 KEY_LIST 的第几个键短按
// 使用示例     uint8 k = key_get_short_press();
// 备注信息     读到短按后会自动清除该键状态（事件消费）
//-------------------------------------------------------------------------------------------------------------------
uint8 key_get_short_press (void)
{
    uint8 i = 0;
    ...
    for(i = 0; i < KEY_NUMBER; i++)
    {
        ...
        {
            key_state[i] = KEY_RELEASE;     // 消费掉这次短按事件，避免重复读取
            return (uint8)(i + 1);          // 1~KEY_NUMBER
        }
    }
    return 0;
}
